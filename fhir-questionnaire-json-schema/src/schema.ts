...
const generateUISchemaItem = (item: QuestionnaireItem): any => {
    ...
    const unitExtension = item.extension?.find(extension => extension.url === 'http://hl7.org/fhir/StructureDefinition/questionnaire-unit');
    if (unitExtension) {
    ...
    if (item.item) {
        ...
        for (const display of displays) {
            const displayExtension = display.extension?.find(e => e.url === 'http://hl7.org/fhir/StructureDefinition/questionnaire-itemControl');
            if (displayExtension?.valueCodeableConcept?.coding?.find(e => e.code === 'help')) {
